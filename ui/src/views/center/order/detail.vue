...
const clinetOrderApi = ClientOrderApi();
...
const onPayment = () => {
	clinetOrderApi.paymentOrder(state.ruleForm.id).then((res) => {
		...
		state.myTimer = setInterval(() => {
			...
			clinetOrderApi.checkOrderPayStatus(state.ruleForm.id).then((res) => {
				if(res.code == 200) {
					closeDialog();
					initForm();
				}
