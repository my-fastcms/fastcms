<template>
	<div >
		<el-dialog title="提现详情" fullscreen v-model="state.isShowDialog">
			<div class="personal">
				<el-row>
					<el-col :xs="24" :sm="24">
										...
										<el-col :span="24">
											<el-row>
												...
												<el-col :xs="24" :sm="4" class="personal-item mb6">
													...
													<div class="personal-item-value">{{state.ruleForm.amount}}</div>
												</el-col>
												<el-col :xs="24" :sm="4" class="personal-item mb6">
													<div class="personal-item-label">收款账号：</div>
													<div class="personal-item-value">{{state.ruleForm.payTo}}</div>
												</el-col>
										...
										<el-col :span="24">
											<el-row>
												...
												<el-col :xs="24" :sm="4" class="personal-item mb6">
													...
													<div class="personal-item-value">{{state.ruleForm.auditTypeStr}}</div>
												</el-col>
											</el-row>
										</el-col>
										<el-col :span="24" v-if="state.ruleForm.feedback != null && state.ruleForm.feedback !=''">
											<el-row>
												<el-col :xs="24" :sm="24" class="personal-item mb6">
													<div class="personal-item-label">拒绝原因：</div>
													<div class="personal-item-value">{{state.ruleForm.feedback}}</div>
												</el-col>
											</el-row>	
										</el-col>
									</el-row>
					...
					<el-col :span="24">
						<el-card shadow="hover" class="mt5 personal-edit" header="最近收入流水">
							<div class="personal-edit-safe-box" v-for="(v, k) in state.ruleForm.userAmountStatementList" :key="k">
			...
			<template #footer>
				<span class="dialog-footer">
                    <el-button type="danger" @click="state.dialogFormVisible = true" v-if="state.ruleForm.status ==0 ? true : false">拒 绝</el-button>
					<el-button type="primary" @click="confirmCashout" v-if="state.ruleForm.status ==0 ? true : false">确认提现</el-button>
					<el-button @click="onCancel">取 消</el-button>
				</span>
			</template>
		</el-dialog>

		<el-dialog v-model="state.dialogFormVisible" title="拒绝提现申请">
			<el-form :model="state.form" :rules="state.rules" ref="myRefForm">
				<el-form-item label="拒绝原因" prop="feedback">
					<el-input v-model="state.form.feedback" />
				</el-form-item>
			</el-form>
			<template #footer>
				<span class="dialog-footer">
...
const state = reactive({
	...
	ruleForm: {
		...
		auditTypeStr: '',
		feedback: '',
		userAmountStatementList: [] as any,
		status: 0
	},
	rules: {
...
const onSubmit = () => {
	...
	myRefForm.value.validate((valid: any) => {
		if (valid) {
			orderApi.refuseCashOut(state.ruleForm.id, qs.stringify(state.form)).then(() => {
				onCancel()
...
const confirmCashout = () => {
	ElMessageBox.confirm('确认提现申请会立即转账给该用户，请认真核对！', '提示', {
		...
		orderApi.confirmCashOut(state.ruleForm.id).then(() => {
			onCancel()
		}).catch(error => {
			ElMessage.error(error.message);
		})
...
const getCashoutDetailInfo = () => {
	if(state.ruleForm.id && state.ruleForm.id != null) {
		orderApi.getCashoutDetail(state.ruleForm.id).then((res) => {
