...
public class FreeMarkerViewPluginRegister extends AbstractPluginRegister implements PluginRegister {
	...
	public void registry(PluginRegistryWrapper pluginRegistryWrapper) throws Exception {
		...
		ClassTemplateLoader classTemplateLoader = new ClassTemplateLoader(pluginRegistryWrapper.getPluginWrapper().getPlugin().getClass(), "");
		List<TemplateLoader> templateLoaders = new ArrayList<>();
		templateLoaders.add(classTemplateLoader);
        templateLoaders.add(new SpringTemplateLoader(new DefaultResourceLoader(pluginRegistryWrapper.getPluginWrapper().getPlugin().getClass().getClassLoader()), "classpath:/templates/"));

		PluginFreeMarkerConfig pluginFreeMarkerConfig = pluginRegistryWrapper.getMainApplicationContext().getBean(PluginFreeMarkerConfig.class);
